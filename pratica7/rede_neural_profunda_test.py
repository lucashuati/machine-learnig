    ...
    def test_sigmoid(self):
        ...
        for i, esp in enumerate(arr_esp):
            self.assertAlmostEqual(
                arr_esp[i],
                a[i],
                msg="Funcao dz_ultima_camada não está com o resultado correto para a função de ativação sigmoid!"
            )

        lst_dz = sigmoid.dz_funcao(
            np.array([-0.4, 0.1, 1]),
            None,
            None,
            np.array([2, 3, -1])
        ...
        for i, esp in enumerate(arr_esp):
            self.assertAlmostEqual(
                arr_esp[i],
                lst_dz[i],
                msg="Funcao dz_funcao não está com o resultado correto para a função de ativação sigmoid!"
            )

    def test_relu(self):
        self.assertListEqual(
            list(relu.funcao(np.array([10, -4, 0.2, 0]))), list(np.array([10, 0, 0.2, 0])))
        lst_dz = relu.dz_funcao(np.array([]), np.array(
            [10, -4, 0.2, 0]), np.array([]), np.array([2, 2, 2, 2]))
        self.assertListEqual(list(lst_dz), list(np.array([2, 0, 2, 2])))

    def test_leaky_relu(self):
        self.assertListEqual(list(leaky_relu.funcao(
            np.array([10, -4, 0.2, 0]))), list(np.array([10, -0.04, 0.2, 0])))

        lst_dz = leaky_relu.dz_funcao(np.array([]), np.array(
            [10, -4, 0.2, 0]), np.array([]), np.array([2, 2, 2, 2]))
        self.assertListEqual(list(lst_dz), list(np.array([2, 0.02, 2, 2])))

    def test_tanh(self):
        # ,np.array([10,-0.04,0.2,0])
        arr_a = tanh.funcao(np.array([10, -4, 0.2, 0]))
        expected = [0.9999999958776927, -
                    0.999329299739067, 0.197375320224904, 0]
        for i, a in enumerate(arr_a):
            self.assertAlmostEqual(
                a, expected[i], msg="Valor inesperado para o calculo da tangente hiperbolica (tanh)")

        arr_dz = tanh.dz_funcao(np.array([]), np.array(
            [10, -4, 0.2, 0]), np.array([]), np.array([2, 2, 2, 2]))
        expected = [1.64892291e-08, 2.68190137e-03,
                    1.92208597e+00, 2.00000000e+00]
        for i, dz in enumerate(arr_dz):
            self.assertAlmostEqual(
                dz, expected[i], msg="Valor inesperado para o calculo da funcao dz da hiperbolica (tanh)")
...
class TestCamada(unittest.TestCase):
    ...
    arr_saida_sigmoid = [np.array([  # entrada->sigmoid
        [0.8284952300245991,	0.7549149868676283,	0.7640475997974661],
        [0.668187772168166,	0.47502081252106,	0.7310585786300049],
        [0.8138161720984914,	0.7455466141264027,
         0.6737070994545215],
        [0.845534734916465,	0.785834983042559,	0.768524783499018],
    ...
    def test_init(self):
        ...
        for i, qtd_unidades in enumerate(arrUnidades):
            ...
            self.assertEqual(len(c.arr_unidades), qtd_unidades,
                             "A quantidade de unidades inesperado!")
            for unid in c.arr_unidades:
                self.assertTrue(np.sum(
                    unid.arr_w) != 0, "Os pesos devem iniciar de forma aleatória, caso contrário, todas as unidades irão ter o mesmo resultado")

    def test_forward_propagation(self):
        for i, mat_entrada in enumerate(TestCamada.arr_entrada):
            camada_sigmoid = Camada(
                TestCamada.arr_saida_sigmoid[i].shape[1], sigmoid.funcao, sigmoid.dz_funcao)
            camada_relu = Camada(
                TestCamada.arr_saida_sigmoid[i].shape[1], relu.funcao, relu.dz_funcao)
            ...
            self.assertListEqual(list(camada_sigmoid.mat_a.shape), list(TestCamada.arr_saida_sigmoid[i].shape), "Dimensões incorretas da matriz de ativações (sigmoid). NUmero de unidades: "+str(
                len(camada_sigmoid.arr_unidades))+" Numero de instancias: "+str(mat_entrada.shape[0]))
            for j in range(camada_sigmoid.mat_a.shape[0]):
                [self.assertAlmostEqual(camada_sigmoid.mat_a[j][k], sig_esperado, msg="Resultado inesperado da matriz de ativações (sigmoid) epserado: "+str(
                    sig_esperado)+" Matriz A de entrada: "+str(mat_entrada)) for k, sig_esperado in enumerate(TestCamada.arr_saida_sigmoid[i][j])]

            self.assertListEqual(list(camada_relu.mat_a.shape), list(TestCamada.arr_saida_relu[i].shape), "Dimensões incorretas da matriz de ativações (relu). NUmero de unidades: "+str(
                len(camada_relu.arr_unidades))+" Numero de instancias: "+str(mat_entrada.shape[0]))
            for j in range(camada_relu.mat_a.shape[0]):
                [self.assertAlmostEqual(camada_relu.mat_a[j][k], relu_esperado, msg="Resultado inesperado da matriz de ativações (relu) esperado: "+str(
                    relu_esperado)+" Matriz A de entrada: "+str(mat_entrada)) for k, relu_esperado in enumerate(TestCamada.arr_saida_relu[i][j])]

    def test_mat_w(self):
        camada_sigmoid = self.inicializa_camada_dz_dw()

        self.assertListEqual(list(camada_sigmoid.mat_w.shape), [
                             2, 3], "O tamanho das dimensões de mat_w está incorreto")
        mat_w_esperado = [[1, 3, 5], [3, 7, 8]]
        for i, arr_w_i in enumerate(camada_sigmoid.mat_w):
            self.assertListEqual(list(
                arr_w_i), mat_w_esperado[i], "Matriz mat_w possui elementos inesperados para a unidade "+str(i))

    def test_mat_dz(self):
        camada_sigmoid = self.inicializa_camada_dz_dw()

        self.assertListEqual(list(camada_sigmoid.mat_dz.shape), [
                             3, 2], "O tamanho das dimensões de mat_dz está incorreto")
        mat_dz_esperado = [[3, 1],
                           [2, 3],
                           [1, 10]]
        for i, arr_dz_i in enumerate(camada_sigmoid.mat_dz):
            self.assertListEqual(list(
                arr_dz_i), mat_dz_esperado[i], "Matriz mat_dz possui elementos inesperados para a unidade "+str(i))
    ...
    def test_mat_dz_w(self):
        ...
        for i, arr_dz_w_i in enumerate(camada_sigmoid.mat_dz_w):
            self.assertListEqual(list(
                arr_dz_w_i), dot_esperado[i], "O calculo da propriedade mat_dz_w possui elementos inesperados para a unidade "+str(i))
    ...
    def inicializa_camada_dz_dw(self):
        ...
        camada_sigmoid.arr_unidades[0].gradiente = Gradiente(
            arr_dz=[3, 2, 1], arr_dw=[3, 2], db=0)
        camada_sigmoid.arr_unidades[1].gradiente = Gradiente(
            arr_dz=[1, 3, 10], arr_dw=[30, 10], db=0)
        camada_sigmoid.qtd_un_camada_ant = 3
    ...
    def test_backward_propagation(self):
        ...
        for i in range(2):
            ...
            for grad_name in grads:
                ...
                grad_sigmoid = camada_sigmoid.arr_unidades[i].gradiente.__dict__[
                    grad_name]
                #grad_relu =  camada_relu.arr_unidades[i].gradiente.__dict__[grad_name]

                self.verifica_grad(
                    grad_sigmoid, grad_exp_sigmoid, grad_name, "sigmoid")
    ...
    def verifica_grad(self, grad, grad_expected, grad_name, nom_ativacao):
        if(type(grad) == list or type(grad) == np.ndarray):
            self.assertListEqual(list(grad.shape), list(
                grad_expected.shape), "A dimensão do gradiente "+grad_name+" está incorreta ("+nom_ativacao+")")

            # verifica o resultado do gradiente
            self.assertListEqual(list(grad), list(grad_expected), "Valor inesperado para o gradiente " +
                                 grad_name+" ("+nom_ativacao+") deveria ser: "+str(grad_expected))
        else:
            self.assertAlmostEqual(grad, grad_expected, msg="Valor inesperado para o gradiente " +
                                   grad_name+"   ("+nom_ativacao+") deveria ser: "+str(grad_expected))
...
class TestRedeNeural(unittest.TestCase):
    ...
    def test_forward_propagation(self):
        self.redeNeural.config_rede(
            TestCamada.arr_entrada[0], np.array([1, 0, 0, 1]))
        self.atualiza_pesos()
        ...
        self.assertListEqual(list(self.redeNeural.arr_camadas[1].mat_a.shape), list(
            expected_final_a.shape), "As dimensões da matriz de ativação (ultima camada) estão incorretas (nao foi testado as anteriores)")
        for i, arr_inst_i in enumerate(self.redeNeural.arr_camadas[1].mat_a):
            self.assertListEqual(list(arr_inst_i), list(expected_final_a[i]), "O valor para instancia "+str(
                i)+" da matriz de ativação da ultima camada não está correta (nao foi testado as anteriores)")
    ...
    def test_backward_propagation(self):
        ...
        for i, unidade in enumerate(self.redeNeural.arr_camadas[0].arr_unidades):
            self.assertAlmostEqual(expected_grads_por_unidade["db"][i], unidade.gradiente.db, msg="O gradiente db (primeira camada, unidade "+str(
                i)+") está incorreto (as demais camadas não foram testadas) Unidade:\n"+str(unidade))
            self.assertListEqual(list(expected_grads_por_unidade["arr_dw"][i].shape), list(
                unidade.gradiente.arr_dw.shape), "As dimensões do gradiente dw (primeira camada) estão incorretas  (as demais camadas não foram testadas) Unidade:\n"+str(unidade))

            self.assertListEqual(list(expected_grads_por_unidade["arr_dw"][i]), list(
                unidade.gradiente.arr_dw), "O gradiente dw (primeira camada, unidade "+str(i)+") está incorreto (as demais camadas não foram testadas) Unidade:\n"+str(unidade))
    ...
    def test_fit(self):
        ...
        self.assertListEqual(expected, list(
            self.redeNeural.arr_camadas[1].arr_unidades[0].arr_a), "A saida final nao está com o valor previsto!")
    ...
    def test_predict(self):
        ...
        self.assertListEqual(
            list(self.redeNeural.predict(mat_x)), [0, 0, 1, 1])
