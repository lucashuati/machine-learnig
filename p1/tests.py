    ...
    def test_entropia(self):
        ...
        for col,entropia_esperada in results.items():
            self.assertAlmostEqual(
                entropia_esperada, #resultado esperado da feature na posicao i, vertcie na posicao posGrafo
        ...
        for col, dic in results.items():
            ...
            for val_atributo,val_gi in results[col].items():
                val_obtido = ganho_informacao_condicional(TestInfoGain.df_teste,
                                            ...
                                            col,val_atributo)
                self.assertAlmostEqual(
                    val_gi, #resultado esperado da feature na posicao i, vertcie na posicao posGrafo
                    val_obtido,#resultado obtido
                    3,# numero de casas decimais que devem ser iguais nesse resultado
                    "GI(A| {at}={val}) deveria ser {esperado} mas foi {obtido}".format(
                        at=col, val=val_atributo, esperado=val_gi, obtido=val_obtido)
                )
